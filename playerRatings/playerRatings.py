...
#region commmands
    ...
    async def addPlayers(self, ctx: Context, *players_to_add):
        ...
        try:
            for playerStr in players_to_add:
                ...
                playerAdded = await self.add_player(ctx, *player)
                if playerAdded:
                    addedCount += 1
                else:
    ...
    @commands.guild_only()
    @checks.admin_or_permissions(manage_guild=True)
    async def addPlayer(self, ctx: Context, member: discord.Member, wins: int, losses: int, elo_rating: int):
        """Add a single player and their info to the file system."""
        playerAdded = await self.add_player(ctx, member, wins, losses, elo_rating)
        if(playerAdded):
            await ctx.send("Done.")
        else:
    ...
    @commands.guild_only()
    @checks.admin_or_permissions(manage_guild=True)
    async def removePlayer(self, ctx: Context, member: discord.Member):
        """Removes player from the file system."""
        playerRemoved = await self.remove_player(ctx, member)
        if playerRemoved:
            await ctx.send("Done.")
        else:
    ...
    @commands.guild_only()
    @checks.admin_or_permissions(manage_guild=True)
    async def clearPlayers(self, ctx: Context):
        """Removes all players from the file system."""
        await self.load_players(ctx.guild)
        players = self.players

        players.clear()
        
        await self._save_players(ctx.guild, players)
        await ctx.send("Done.")

    @commands.guild_only()
    @commands.command(aliases=["rr", "reportresult"])
    async def reportResult(self, ctx: Context, member_1: discord.Member, member_1_wins: int, member_2_wins: int, member_2: discord.Member):
        ...
        if not self._self_report_flag(ctx.guild):
    ...
    @checks.admin_or_permissions(manage_guild=True)
    async def adminReportResults(self, ctx: Context, *match_results):
        ...
        try:
            for matchStr in match_results:
                ...
                matchSubmitted = await self.admin_report_result(ctx, *match)
                if matchSubmitted:
                    addedCount += 1
                else:
    ...
    @checks.admin_or_permissions(manage_guild=True)
    async def adminReportResult(self, ctx: Context, member_1: discord.Member, member_1_wins: int, member_2_wins: int, member_2: discord.Member):
        ...
        if await self.admin_report_result(ctx, member_1, member_1_wins, member_2_wins, member_2):
            await ctx.send("Done.")

    @commands.guild_only()
    @commands.command(aliases=["pi"])
    async def playerInfo(self, ctx: Context, member: discord.Member = None):
    ...
    async def playerLeaderboard(self, ctx: Context, tier = None):
        ...
        await ctx.send(embed=self.embed_leaderboard(ctx.guild, players, tier_role))

    @commands.guild_only()
    @commands.command(aliases=["toggleReport", "toggleSelfReporting", "toggleSR", "toggleselfreport", "togglesr", "tsr"])
    @checks.admin_or_permissions(manage_guild=True)
    async def toggleSelfReport(self, ctx: Context):
        """
    ...
    @checks.admin_or_permissions(manage_guild=True)
    async def getAllPlayers(self, ctx: Context):
        await self.load_players(ctx.guild)
        players = self.players
        if not players:
...
#region helper methods
    ...
    async def add_player(self, ctx: Context, member, wins, losses, elo_rating):
        await self.load_players(ctx.guild)
        players = self.players
    ...
    async def remove_player(self, ctx: Context, member: discord.Member):
        await self.load_players(ctx.guild)
        players = self.players
    ...
    async def admin_report_result(self, ctx: Context, member_1: discord.Member, member_1_wins: int, member_2_wins: int, member_2: discord.Member):
        await self.load_players(ctx.guild)
        player_1 = self.get_player_by_id(self.players, member_1.id)
        if not player_1:
            await ctx.send("There was a problem finding player info for {}. Please verify that you have the correct member in your command. If this persists message an admin.".format(member_1.name))
    ...
    async def verify_game_results(self, ctx: Context, member_1: discord.Member, member_2: discord.Member, member_1_wins: int, member_2_wins: int, verifier: discord.Member):
    ...
    async def finish_game(self, ctx: Context, player_1, player_2, player_1_wins: int, player_2_wins: int):
        ...
        await self._save_players(ctx.guild, self.players)
    ...
    async def get_player_record_and_rating_by_id(self, guild: discord.Guild, member_id):
        await self.load_players(guild)
        player = self.get_player_by_id(self.players, member_id)
        if player:
            return (player.wins, player.losses, player.elo_rating)
        return None

    async def guild_has_players(self, guild: discord.Guild):
        await self.load_players(guild)
        if self.players:
            return True
        return False

    async def get_player_seed(self, ctx: Context, user_team_name):
        user = ctx.author
        if not self.team_manager.is_subbed_out(user):
            ...
            sorted_members = await self.sort_members_by_rating(ctx.guild, active_members)
            try:
    ...
    async def get_member_by_team_and_seed(self, ctx: Context, team_name, seed):
        active_members = await self.team_manager.get_active_members_by_team_name(ctx, team_name)
        sorted_members = await self.sort_members_by_rating(ctx.guild, active_members)
        return sorted_members[seed - 1]

    async def get_ordered_opponent_names_and_seeds(self, ctx: Context, seed, is_home, opposing_team_name):
        ...
        sorted_opponents = await self.sort_members_by_rating(ctx.guild, active_opponents)
        if is_home:
            if seed == 1:
    ...
    async def sort_members_by_rating(self, guild: discord.Guild, member_list):
        await self.load_players(guild)
        if not self.players:
            return member_list
        players = []
    ...
    async def set_player_temp_rating(self, guild: discord.Guild, subbed_member, subbed_out_member):
        await self.load_players(guild)
        if self.players:
            ...
            if subbed_player and subbed_out_player:
                ...
                await self._save_players(guild, self.players)
                return True
        return False

    async def reset_temp_rating(self, guild: discord.Guild, member):
        await self.load_players(guild)
        if self.players:
            player = self.get_player_by_id(self.players, member.id)
            if player:
...
#region embed methods
    ...
    def embed_leaderboard(self, guild: discord.Guild, sorted_players, tier_role):
        embed_color = discord.Colour.blue()
        embed_name = guild.name
        if tier_role:
            embed_color = tier_role.color
...
#region load/save methods
    ...
    async def load_players(self, guild: discord.Guild):
        players = await self._players(guild)
        player_list = []
        remove_player = False
    ...
    async  def _save_players(self, guild: discord.Guild, players):
        ...
        await self.config.guild(guild).Players.set(player_dict)

    async def _toggle_self_report_flag(self, guild: discord.Guild):
    ...
    async def _self_report_flag(self, guild: discord.Guild):
        return await self.config.guild(guild).SelfReportFlag()

#endregion

class Player:
    def __init__(self, member: discord.Member, wins: int, losses: int, elo_rating: int, temp_rating: int = -1):
        self.member = member
        self.wins = wins
        self.losses = losses
